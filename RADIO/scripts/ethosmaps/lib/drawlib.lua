...
function drawLib.drawStatusBar(widget,maxRows)
  ...
  lcd.color(status.colors.barBackground)
  lcd.pen(SOLID)
  local statusBarY = status.widgetHeight - yDelta
  lcd.drawFilledRectangle(0, statusBarY, status.widgetWidth, yDelta)
  -- messages
  lcd.font(FONT_STD)
...
function drawLib.drawFailsafe(widget)
  if status.telemetry.ekfFailsafe > 0 then
    local failsafeX = math.floor(status.widgetWidth / 2) - 30
    local failsafeY = math.floor(status.widgetHeight / 2) - 60
    drawLib.drawBlinkBitmap(failsafeX, failsafeY, "ekffailsafe")
  elseif status.telemetry.battFailsafe > 0 then
    local failsafeX = math.floor(status.widgetWidth / 2) - 30
    local failsafeY = math.floor(status.widgetHeight / 2) - 60
    drawLib.drawBlinkBitmap(failsafeX, failsafeY, "battfailsafe")
  elseif status.telemetry.failsafe > 0 then
    local failsafeX = math.floor(status.widgetWidth / 2) - 30
    local failsafeY = math.floor(status.widgetHeight / 2) - 60
    drawLib.drawBlinkBitmap(failsafeX, failsafeY, "failsafe")
  end
end
...
function drawLib.drawNoTelemetryData(widget)
  if not libs.utils.telemetryEnabled() then
    ...
    local boxH = math.floor(textH + 55*sy)          -- deutlich höher, damit Text nicht abgeschnitten wird
...
function drawLib.drawNoGPSData(widget)
  ...
  local boxW = math.min(math.floor(textW + 80*sx), math.floor(w * 0.88))
  local boxH = math.floor(textH + 55*sy)          -- deutlich höher
  local boxX = math.floor((w - boxW) / 2)
  local boxY = math.floor((h / 2) - boxH / 2) - 15*sy
...
function drawLib.drawTopBarSensor(widget, x, sensor, label)
  ...
  -- Name auf max. 4 Zeichen kürzen bei Split-Screen
  ...
  -- 2 Pixel enger: Name → Wert und Widget → Widget
  ...
  return valW + lblW + 10   -- war 12, jetzt 2 px enger
end
...
function drawLib.drawTopBar(widget)
  ...
  -- Modellname nur bei breiten Widgets anzeigen
  if w >= 450 then
    drawLib.drawText(8*sx, 2*sy, status.modelString or model.name(), FONT_L, status.colors.barText, LEFT)
  end

  -- Sensoren von rechts nach links (TX + Link + User-Sensoren)
  local offset = 12*sx
